export class Point {
  ...
  plus (that: Point) {
    return new Point(
      this.x + that.x * this.scale,
      this.y + that.y * this.scale,
      this.scale * that.scale
    )
  }
  times (e: number) {
    if (this.scale === 1) return new Point(this.x * e, this.y * e, 1)
    if (e === 1) return this
  ...
  eq (that: Point) {
    return (
      eq(this.x, that.x) && eq(this.y, that.y) && eq(this.scale, that.scale)
    )
  }

  distanceFrom (that: Point, base = Math.E) {
    return Math.hypot(
      this.x - that.x,
      this.y - that.y,
      (Math.log(this.scale) - Math.log(that.scale)) / Math.log(base)
    )
  }

  /** scale the point from an origin according to the Poincaré metric */
  poincareInterpolateFrom (
    origin: Point,
    height: number,
    rho: number
  ): (t: number) => Point {
    ...
    const ux0 = origin.x,
      uy0 = origin.y,
      w0 = origin.scale * height,
      ux1 = this.x,
      uy1 = this.y,
      w1 = this.scale * height
    const dx = ux1 - ux0,
      dy = uy1 - uy0,
      d2 = dx * dx + dy * dy,
      d1 = sqrt(d2)
    ...
    if (isNaN(dr)) {
      ...
      if (S === 0) return _ => origin
      return x =>
        new Point(ux0 + dx * x, uy0 + dy * x, (w0 * exp(rho * S * x)) / height)
    }
    const S = dr / rho
   ...
   */
  poincareDistanceFrom (that: Point, height: number, rho: number): number {
    const { asinh, acosh, sinh, cosh, exp, log, sqrt, tanh } = Math
    // code from d3.js interpolateZoom
    const rho2 = rho * rho,
      rho4 = rho2 * rho2
